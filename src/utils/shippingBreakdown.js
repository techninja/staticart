...
function calcSubtotals(shipping, items) {
  ...
  for (const item of items) {
    ...
    if (!matched && shipping.defaultClass) subs[shipping.defaultClass] += item.price * item.quantity;
  }
  return subs;
}
...
function buildTableHtml(cls, tiers, sub, rKeys, activeR) {
  ...
  for (let i = 0; i < tiers.length; i++) {
    if (tiers[i].upTo && sub < tiers[i].upTo) { activeTier = i; break; }
    if (!tiers[i].upTo) activeTier = i;
  }
  const ths = rKeys.map((r) =>
    `<th class="${r === activeR ? 'active-region' : ''}">${REGION_LABELS[r] || r}</th>`,
  ).join('');
  const rows = tiers.map((t, i) => {
    const tierLabel = t.upTo ? `Under ${formatPrice(t.upTo)}` : `${formatPrice(tiers[i - 1]?.upTo || 0)}+`;
    const tds = rKeys.map((r) => {
  ...
  return `<h4>${label} (${formatPrice(sub)} in cart)</h4>`
...
export function renderShippingBreakdown(items, region) {
  ...
  const tableHtmls = Object.entries(shipping.tiers || {})
    ...
    .map(([cls, tiers]) => buildTableHtml(cls, /** @type {any[]} */ (tiers), subs[cls], regionKeys, regionKey));
  if (!tableHtmls.length) return html``;
  const combined = tableHtmls.map((t) => `<div class="shipping-breakdown__table">${t}</div>`).join('');
  return html`
    <div class="shipping-breakdown">
      <details class="shipping-breakdown__details">
        ...
        <p class="shipping-breakdown__note">
          Shipped via USPS Priority or First Class. Orders with posters/prints
          include tube packaging. Book and tube rates are added together.
        </p>
