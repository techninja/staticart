 ...
 *
 * Usage: node scripts/cache-bust.js [--hash <custom>] [--root <project-root>]
 * Without --hash, uses a short timestamp-based hash.
 * Without --root, resolves from script location (works when script is in project/scripts/).
...
html = html.replace(/(\.(css|js|json))\?v=[^"]+/g, '$1');
